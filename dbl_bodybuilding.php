...
function handleFormSubmission($conn) {
    // Prepare and bind the statement for inserting data into bodybuilding_registration table
    $stmt_insert = $conn->prepare("INSERT INTO bodybuilding_registration (name, email, phone, age, gender, experience, goals, health_conditions, workout_frequency, dietary_restrictions, preferred_training_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt_insert->bind_param("sssisssssss", $name, $email, $phone, $age, $gender, $experience, $goals, $health_conditions, $workout_frequency, $dietary_restrictions, $preferred_training_time);
    ...
    $workout_frequency = $_POST['workout_frequency'];
...
function createOrUpdateView($conn) {
    // Create or update the bodybuilding_registration_view
    $sql_create_view = "CREATE OR REPLACE VIEW bodybuilding_registration_view AS SELECT id, name, email, age, gender, goals FROM bodybuilding_registration";
...
function displayRegisteredUsers($conn) {
    ...
    $sql_view = "SELECT * FROM bodybuilding_registration_view";
